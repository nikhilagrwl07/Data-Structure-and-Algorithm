...
//        insertInBST(root,7);
//        insertInBST(root,5);
//      declareArray(sizeOfTree(root));

        // System.out.println("Size = "+inOrder.length);
...
//---------------------------------------------------------
        ...
        prune(root, k);
        LevelOrder();
//       LevelOrderUsing2Stack(root);
        //   root=deletePathslessSum(root, k);

        //rInOrderModifed(root);
...
//      //isBST(root);
        ...
        // kthSmallestNode_kthLargestNode_();
        //printRange();
    }
    ...
    public void insertInBST(Node r, int x) {
        if (x < r.data) {
            if (r.left == null) {
                r.left = new Node(x, null, null);
            } else {
                insertInBST(r.left, x);
            }
        } else if (x >= r.data) {
            if (r.right == null) {
                r.right = new Node(x, null, null);
            } else {
...
//        }
    ...
    public int diameter_same_recursion(Node root) {
        ...
        return datatracker; //getMaxLength();
    }
...
// Source: https://github.com/monkeylyf/interviewjam/blob/master/tree/Remove_All_Nodes_Do_Not_Lie_In_Any_Path_With_Sum_More_Than_K.java
    ...
    public  Node pruneUtil(Node root, int k, intWrapper sum) {
        if (root == null) {
            return null;
        }
    ...
    public void printRangeUtil(Node r, int k1, int k2) {
     ...
     public void printtupleForSum(int sum)
     {
        ...
        while(!tempArrayForQueue.isEmpty())
        {
            ...
            if(searchElementUtil(temp,r) && l!=((int) h.get(r)))
            {
