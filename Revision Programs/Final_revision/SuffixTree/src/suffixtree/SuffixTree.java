 ...
 * Active Edge - It is used to choose the edge from active node. It has suffixIndex of character. 
...
public class SuffixTree {
    ...
    public static void main(String args[]){
        ...
        st.build();
         st.dfsTraversal();
       // st.getLongestRepeatedSubString(s.toCharArray());
       
       // System.out.println(st.validate());
        
    }
    
    private static SuffixNode root;
    ...
    public void getLongestRepeatedSubString(char[] text)
    {
        System.out.println("LRS ::: of String "+ String.valueOf(text));
        
        doTraversal(root, 0);
         
         System.out.println("substringStartIndex :: " + substringStartIndex);
      int k;
    for (k=0; k<maxHeight; k++)
    {
        System.out.print(text[k + substringStartIndex]);
    }
    if(k == 0)
        System.out.print("No repeated substring");
   
    System.out.println("");
    }
    
    
   public void doTraversal(SuffixNode n, int labelHeight)
   {
       if(n==null) return;
       
       System.out.println("----> " + n.suffixIndex +" :: " + labelHeight);
       int i=0;
       if(n.suffixIndex==-1)
       {
           for(i=0;i<256;i++)
           {
             if(n.child[i] != null)
            {
               doTraversal(n.child[i], labelHeight +edgeLength(n.child[i]));
            }
           }
       }
   ...
   public int edgeLength(SuffixNode n) {
    if(n == root)
        return 0;
    ...
    public void build(){
        ...
        //finally walk the tree again and set up the suffixIndex.
            ...
            if(active.activeLength == 0){
                ...
                try {
                    ...
                    if(ch == input[i]){
                        ...
                        //if active node is root then increase active suffixIndex by one and decrease active length by 1
                        else{
    ...
    public void dfsTraversal(){
        ...
        for(SuffixNode node : root.child){
            ...
            if(node!=null)
                System.out.println("root.child.start :: " + node.start + " :: " + node.end.end);
    ...
    private void dfsTraversal(SuffixNode root, List<Character> result){
        ...
        System.out.println("root.start :: " + root.start + "  root.end :: " + root.end.end);
        System.out.println("suffix index " + root.suffixIndex);
        if(root.suffixIndex != -1){
      ...
      //   System.out.println("");
            ...
            System.out.println(" " + root.suffixIndex);
            for(int i=root.start; i <= root.end.end; i++){
    ...
    /**
     * Do validation of the tree by comparing all suffixes and their suffixIndex at leaf node.
     */
